...
#define ImGuiToastType_Info 2
#define ImGuiToastType_None 3
#define ImGuiToastType_Warning 4
#define ImGuiToastType_Error 5
#define ImGuiToastType_Launch 6
	...
	NOTIFY_INLINE auto get_default_title() -> const char*
	{
		if (!strlen(this->title))
		{
			...
			case ImGuiToastType_Info:
				return "Info";
			case ImGuiToastType_Launch:
				return "[NKERNEL] Launch Success";
			default:
				return NULL;
			}
		}
...
namespace ImGui
{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			// Here we render the toast content
			{
				...
				if (NOTIFY_USE_DISMISS_BUTTON)
				{
					...
					ImVec4 bgColor = GetStyleColorVec4(ImGuiCol_WindowBg);
					PushStyleColor(ImGuiCol_Button, ImVec4(bgColor));
					PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(bgColor));
					PushStyleColor(ImGuiCol_ButtonActive, ImVec4(bgColor));
					//	PushStyleColor(ImGuiCol_Button, bgColor);
			//			PushStyleColor(ImGuiCol_ButtonHovered, bgColor);
			//			PushStyleColor(ImGuiCol_ButtonActive, bgColor);
	...
	NOTIFY_INLINE void MergeIconsWithLatestFont(float font_size, bool FontDataOwnedByAtlas = false)
	{
		...
		GetIO().Fonts->AddFontFromMemoryTTF((void*)fa_solid_900_compressed_data, fa_solid_900_compressed_size, font_size, &icons_config, icons_ranges);
	}
}
